...
export default function PostDateTags({ post, showFull = false, ...props }) {
  return (
    <Box fontSize="xs" fontWeight="600" letterSpacing="widest" textTransform="uppercase" {...props}>
      {showFull && (
        <>
          {format(parseISO(post.date), `${showFull ? 'EEEE, ' : ''}d MMMM yyy`)} &middot; Updated{' '}
          {format(parseISO(post.updatedAt), 'd MMMM yyy, hh:mm a')} &middot;{' '}
        </>
      )}
      <Box d="inline-block" sx={{ a: { ':not(:last-of-type):after': { content: '", "' } } }}>
        {post.tags.map((tag) => (
