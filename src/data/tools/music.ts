interface Logo {
  image: string;
  title: string;
  link: string;
}
export interface Tool {
  name: string;
  desc: string;
  logos: Logo[];
}

const musicTools: Tool[] = [
  {
    name: 'Workstation',
    desc: 'i9-9900K 64GB Hackintosh (December 2019 build) &amp; MacBook Pro 15&quot; 2018 (homestudio) &amp; iMac Retina 2015 (studio)',
    logos: [
      {
        image: '/assets/images/logos/apple.svg',
        title: 'Apple',
        link: 'https://apple.com',
      },
      {
        image: '/assets/images/logos/intel.svg',
        title: 'Intel',
        link: 'https://intel.com',
      },
    ],
  },
  {
    name: 'Digital Audio Workstation',
    desc: 'Logic Pro X 10.7, Avid Pro Tools 12, PreSonus Studio One 5 &amp; Cubase Pro 11',
    logos: [
      {
        image: '/assets/images/logos/apple.svg',
        title: 'Apple',
        link: 'https://apple.com',
      },
      {
        image: '/assets/images/logos/avid.svg',
        title: 'Avid',
        link: 'https://avid.com',
      },
      {
        image: '/assets/images/logos/presonus.svg',
        title: 'PreSonus',
        link: 'https://presonus.com',
      },
      {
        image: '/assets/images/logos/steinberg.svg',
        title: 'Steinberg',
        link: 'https://steinberg.net',
      },
    ],
  },
  {
    name: 'Gears',
    desc: 'Interfaces: RME Fireface UFX II (Main interface), UAD Apollo Twin X Quad, Apogee Duet 2, Focusrite Clarett 8Pre USB. Preamps: Warm Audio WA-73EQ. Mics: Neumann TLM103, Slate Digital VMS ML-1, Audio Technica AT2035, Aston Origin. Controllers: Arturia KeyLab 88 MkII, Native Instruments Komplete Kontrol A61 & M32, Roli Seaboard Block, Compressors: Alesis 3630, dbx 166xs. Monitor controller: Mackie Big Knob Studio.',
    logos: [
      {
        image: '/assets/images/logos/rme.svg',
        title: 'RME',
        link: 'https://www.rme-audio.de',
      },
      {
        image: '/assets/images/logos/uad.svg',
        title: 'Universal Audio',
        link: 'https://uaudio.com',
      },
      {
        image: '/assets/images/logos/warm-audio.svg',
        title: 'Warm Audio',
        link: 'https://warmaudio.com',
      },
      {
        image: '/assets/images/logos/neumann.svg',
        title: 'Neumann',
        link: 'https://en-de.neumann.com/',
      },
      {
        image: '/assets/images/logos/slate.svg',
        title: 'Slate Digital',
        link: 'https://slatedigital.com/',
      },
      {
        image: '/assets/images/logos/apogee.svg',
        title: 'Apogee',
        link: 'https://apogeedigital.com',
      },
      {
        image: '/assets/images/logos/ni.svg',
        title: 'Native Instruments',
        link: 'https://native-instruments.com',
      },
      {
        image: '/assets/images/logos/focusrite.svg',
        title: 'Focusrite',
        link: 'https://focusrite.com',
      },
      {
        image: '/assets/images/logos/presonus.svg',
        title: 'PreSonus',
        link: 'https://presonus.com',
      },
      {
        image: '/assets/images/logos/dbx.svg',
        title: 'dbx',
        link: 'https://dbxpro.com',
      },
      {
        image: '/assets/images/logos/mackie.svg',
        title: 'Mackie',
        link: 'https://mackie.com',
      },
      {
        image: '/assets/images/logos/audio-technica.svg',
        title: 'Audio Technica',
        link: 'https://audio-technica.com',
      },
      {
        image: '/assets/images/logos/roli.svg',
        title: 'ROLI',
        link: 'https://roli.com',
      },
    ],
  },
  {
    name: 'Studio Monitors',
    desc: 'Studio: Yamaha HS8 &amp; NS10 studio (the legend). Home: Yamaha HS80M, Adam Audio T7V, IK Multimedia iLoud',
    logos: [
      {
        image: '/assets/images/logos/yamaha.svg',
        title: 'Yamaha',
        link: 'https://focusrite.com',
      },
      {
        image: '/assets/images/logos/adam-audio.svg',
        title: 'Adam Audio',
        link: 'https://adam-audio.com',
      },
      {
        image: '/assets/images/logos/tannoy.svg',
        title: 'Tannoy',
        link: 'https://tannoy.com',
      },
    ],
  },
  {
    name: 'Go-to software instruments',
    desc: 'Native Instruments Kontakt 6, Spectrasonics Omnisphere 2, Toontrack Superior Drummer 3, Xfer Serum, LennarDigital Sylenth1, Synapse Audio Dune 3, Logic&apos;s Alchemy',
    logos: [
      {
        image: '/assets/images/logos/ni.svg',
        title: 'Native Instruments',
        link: 'https://native-instruments.com',
      },
      {
        image: '/assets/images/logos/spectrasonics2.svg',
        title: 'Spectrasonics',
        link: 'https://www.spectrasonics.net',
      },
      {
        image: '/assets/images/logos/toontrack.svg',
        title: 'Toontrack',
        link: 'https://toontrack.com',
      },
    ],
  },
  {
    name: 'Go-to mix plugins',
    desc: 'Fabfilter (Pro-Q3, Pro-R, Pro-DS, Pro-L), UAD (Avalon VT-737sp, Neve 1073, LA-2A, 1176, Fairchild 660/670, Distressor, Capitol Chambers, Lexicon 480L, EMT 140), Slate Digital (VMR, VMC), Waves (C6, API stuff, SSL stuff, Scheps Channel Strip, CLA stuff, Abbey Road Plates/Chambers), SoundTheory Gullfoss, SoundToys (Echoboy, Decapitator, Devil-Loc, Radiator, Microshift), Plugin Alliance (bx_consoles, bx_townhouse Buss Compressor, Shadow Hills Compressor, MAAG EQ), Native Instruments Guitar Rig 6, ValhallaDSP Reverbs/Delays, Oeksound Soothe 2, Denise Audio Perfect Room.',
    logos: [
      {
        image: '/assets/images/logos/fabfilter.svg',
        title: 'Fabfilter',
        link: 'https://fabfilter.com',
      },
      {
        image: '/assets/images/logos/uad.svg',
        title: 'Universal Audio',
        link: 'https://uaudio.com',
      },
      {
        image: '/assets/images/logos/slate.svg',
        title: 'Slate Digital',
        link: 'https://slatedigital.com/',
      },
      {
        image: '/assets/images/logos/waves.svg',
        title: 'Waves',
        link: 'https://waves.com',
      },
      {
        image: '/assets/images/logos/soundtoys.svg',
        title: 'SoundToys',
        link: 'https://soundtoys.com/',
      },
      {
        image: '/assets/images/logos/plugin-alliance.svg',
        title: 'Plugin Alliance',
        link: 'https://plugin-alliance.com',
      },
      {
        image: '/assets/images/logos/ni.svg',
        title: 'Native Instruments',
        link: 'https://native-instruments.com',
      },
      {
        image: '/assets/images/logos/valhalla.svg',
        title: 'ValhallaDSP',
        link: 'https://valhalladsp.com',
      },
      {
        image: '/assets/images/logos/oeksound.svg',
        title: 'Oeksound',
        link: 'https://oeksound.com',
      },
    ],
  },
];

export default musicTools;
